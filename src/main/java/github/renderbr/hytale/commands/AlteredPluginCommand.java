...
public class AlteredPluginCommand extends AbstractCommandCollection {
    @Nonnull
    ...
    private static class PluginListCommand extends CommandBase {
        private final FlagArg displayBasePlugins;
        ...
        protected void executeSync(@Nonnull CommandContext context) {
            ...
            plugins = (Set) module.getPlugins().stream().filter((b) -> !b.getIdentifier().getGroup().contains("Hytale")).map(PluginBase::getIdentifier).map(PluginIdentifier::toString).map(Message::raw).collect(Collectors.toSet());
            context.sendMessage(MessageFormat.list(Message.translation("server.commands.plugin.plugins"), plugins));
        }
    }
    ...
    private static class PluginLoadCommand extends CommandBase {
        ...
        public PluginLoadCommand() {
            ...
            this.pluginNameArg = this.withRequiredArg("pluginName", "server.commands.plugin.load.pluginName.desc", PLUGIN_IDENTIFIER_ARG_TYPE);
    ...
    private static class PluginUnloadCommand extends CommandBase {
        ...
        public PluginUnloadCommand() {
            ...
            this.pluginNameArg = this.withRequiredArg("pluginName", "server.commands.plugin.unload.pluginName.desc", PLUGIN_IDENTIFIER_ARG_TYPE);
    ...
    private static class PluginReloadCommand extends CommandBase {
        ...
        public PluginReloadCommand() {
            ...
            this.pluginNameArg = this.withRequiredArg("pluginName", "server.commands.plugin.reload.pluginName.desc", PLUGIN_IDENTIFIER_ARG_TYPE);
